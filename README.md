...
**线上地址**: https://ai-signal-monitor.chinahaozai.workers.dev

分析框架见 [`FRAMEWORK.md`](./FRAMEWORK.md) —— 信号权重表、三态判定规则、AI 分析口径都以该文档为准。
...
代码本身零 npm 依赖,`git clone` + Node ≥ 18 就能跑 `npx wrangler dev`。以下三样是本地状态,不进 git,换电脑要单独处理:

1. **`.dev.vars`** (项目根) —— 存 `SAMPLE_TOKEN`、`DEEPSEEK_API_KEY`。
   两条路子:
   - 旧电脑 `scp .dev.vars 新电脑:项目根/` 迁过来
   - 或者去 Cloudflare secret / DeepSeek 控制台重新取,在新电脑手动新建同名文件
2. **Cloudflare 账号授权** (`~/Library/Preferences/.wrangler/config/default.toml`) ——
   **不用迁**。新电脑跑一次 `npx wrangler login` 浏览器授权即可重生成。
3. **`.wrangler/`** (项目根) —— 本地 dev 的 KV 模拟缓存。**不用管**,自动重建。

浏览器端还有一样要重来:**AI 分析用的 DeepSeek key 存在浏览器 localStorage,不跟账号走**。
换电脑打开线上地址后,右上齿轮里重新填一次 key。这不是 bug,是 BYOK 架构的固有代价。
